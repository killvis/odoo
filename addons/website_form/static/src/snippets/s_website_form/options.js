...
const FormEditor = options.Class.extend({
     ...
     * @param {string} mark
     */
    _setLabelsMark: function (mark) {
        if (!mark) {
            mark = this._getMark();
        }
        let markSelector, unmarkSelector;
        const requiredSelector = '.o_website_form_model_required .o_we_form_label, .o_website_form_required .o_we_form_label';
        const optionalSelector = '.s_website_form_field:not(.o_website_form_model_required):not(.o_website_form_required) .o_we_form_label';
        if (this._isRequiredMark()) {
            markSelector = requiredSelector;
            unmarkSelector = optionalSelector;
        } else if (this._isOptionnalMark()) {
            markSelector = optionalSelector;
            unmarkSelector = requiredSelector;
        } else {
            unmarkSelector = '.s_website_form_field';
        }
        this.$target[0].querySelectorAll(unmarkSelector).forEach(label => {
            let span = label.querySelector('span.o_website_form_mark');
            if (span) {
                span.remove();
            }
        });
        this.$target[0].querySelectorAll(markSelector).forEach(label => {
            let span = label.querySelector('span.o_website_form_mark');
            if (!mark) {
                if (span) {
                    span.remove();
    ...
    _getDefaultFormat: function () {
        ...
        return {
            ...
            optionalMark: this._isOptionnalMark(),
            mark: this._getMark(),
        };
    }
});
...
const FieldEditor = FormEditor.extend({
    ...
    _getCustomField: function (type, name) {
        return {
            ...
            formatInfo: this._getDefaultFormat(),
        };
    },
    /**
    ...
    _getFieldName: function () {
        return this.$target[0].querySelector('.o_we_form_label').htmlFor;
    },
    /**
     ...
     */
    _getMark: function () {
        return this.formEl.dataset.mark;
    },
    /**
     * @override
     */
    _isRequiredMark: function () {
        return this.formEl.classList.contains('o_mark_required');
     ...
     */
    _isOptionnalMark: function () {
        return this.formEl.classList.contains('o_mark_optional');
    },
    /**
    ...
    _isFieldRequired: function () {
        ...
        return classList.contains('o_website_form_required') || classList.contains('o_website_form_model_required');
    },
    /**
    ...
    */
    ...
    _computeWidgetVisibility: function (widgetName, params) {
        switch (widgetName) {
            ...
            case 'hidden_opt':
            case 'type_opt':
                return !this.$target[0].classList.contains('o_website_form_model_required');
        }
        return this._super(...arguments);
    },
    /**
    ...
    _renderCustomXML: function (uiFragment) {
        ...
        const fieldsInForm = Array.from(this.formEl.querySelectorAll('.s_website_form_field:not(.o_website_form_custom)')).map(field => field.dataset.type).filter(el => el !== fieldType);
            ...
            this.listTable.querySelectorAll('input').forEach(el => {
                const params = {
                    field: {
                        ...
                        vertical: multiInputsWrap.dataset.display === 'vertical',
                    }
                };
    ...
    _getMultipleInputs: function () {
        return this.$target[0].querySelector('.o_website_form_multiple');
    },
    /**
...
options.registry.WebsiteFormFieldModel = DisableOverlayButtonOption.extend({
    start: function () {
        this.disableButton('clone', 'You can\'t duplicate a model field.');
...
options.registry.WebsiteFormFieldRequired = DisableOverlayButtonOption.extend({
    start: function () {
        this.disableButton('remove', 'You can\'t remove a field that is required by the model itself.');
        return this._super.apply(this, arguments);
    }
});
...
options.registry.WebsiteFormSubmitRequired = DisableOverlayButtonOption.extend({
    start: function () {
        this.disableButton('remove', 'You can\'t remove the submit button of the form');
        this.disableButton('clone', 'You can\'t duplicate the submit button of the form.');
        return this._super.apply(this, arguments);
    }
});
});
