...
class UoM(models.Model):
    ...
    def _onchange_uom_category(self):
        if len(self.env['uom.uom'].search([('category_id', '=', self.category_id.id), ('uom_type', '=', 'reference')]).ids) == 1 and self.uom_type == 'reference':
    ...
    def unlink(self):
        if self.filtered(lambda uom: uom.category_id in (self.env.ref('uom.product_uom_categ_unit'), self.env.ref('uom.uom_categ_wtime'))) and self.uom_type == 'reference':
