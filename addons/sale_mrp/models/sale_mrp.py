...
class MrpProduction(models.Model):
    ...
    mto_sale_order_count = fields.Integer("Count of Source SO", compute='_compute_mto_sale_order_count', store=True)
    ...
    def action_view_mto_sale_orders(self):
        ...
        return {
            ...
            'domain': [('id', 'in', self.move_dest_ids.group_id.sale_id.ids)],
...
class SaleOrder(models.Model):
    ...
    mto_mrp_production_count = fields.Integer("Count of MO generated", compute='_compute_mto_mrp_production_count', store=True)

    @api.depends('procurement_group_id.related_move_ids.created_production_id')
    def _compute_mto_mrp_production_count(self):
        for sale in self:
            sale.mto_mrp_production_count = len(sale.procurement_group_id.related_move_ids.created_production_id)

    def action_view_mto_mrp_production(self):
        self.ensure_one()
        return {
            'name': _('Manufacturing Orders Generated by %s' % self.name),
            'domain': [('id', 'in', self.procurement_group_id.related_move_ids.created_production_id.ids)],
